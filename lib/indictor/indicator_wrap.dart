  ...
  double _calculateScrollOffset() {
    return (floating
            ? (mode == RefreshStatus.twoLeveling ||
                ...
                ? AGRefresher.ofState(context).viewportExtent
                : widget.height)
            : 0.0) -
        _position?.pixels;
  }
  ...
  void _handleModeChange() {
    ...
    if (mode == RefreshStatus.idle || mode == RefreshStatus.canRefresh) {
      ...
      if (mode == RefreshStatus.idle)
        AGRefresher.ofState(context).setCanDrag(true);
    }
    if (mode == RefreshStatus.completed || mode == RefreshStatus.failed) {
      ...
      floating = true;
      AGRefresher.ofState(context).setCanDrag(false);
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
...
mixin IndicatorStateMixin<T extends StatefulWidget, V> on State<T> {
  ...
  void _updateListener() {
    ...
    refresher = AGRefresher.of(context);
  ...
  void initState() {
    // TODO: implement initState
    if (V == RefreshStatus) {
      AGRefresher.of(context)?.controller?.headerMode?.value =
          RefreshStatus.idle;
    }
    super.initState();
  }
