...
import '../effect/refresh_physics.dart';
import 'indicator_wrap.dart';
import '../effect/sliver.dart';
import './classic_indictor.dart';
...
abstract class IndicotorStatus {
...
class AGRefresher extends StatefulWidget {
 ...
 AGRefresher(
  ...
  @override
  AGRefresherState createState() => AGRefresherState();

  static AGRefresher of(BuildContext context) {
    return context?.findAncestorWidgetOfExactType<AGRefresher>();
  }

  static AGRefresherState ofState(BuildContext context) {
    return context?.findAncestorStateOfType<AGRefresherState>();
  }
...
class AGRefresherState extends State<AGRefresher> {
  RefreshPhysics _physics;
  bool _updatePhysics = false;
  double viewportExtent = 0;
  bool _canDrag = true;

   final RefreshIndicator defaultHeader = ClassicHeader();
  ...
  @override
  void didUpdateWidget(AGRefresher oldWidget) {
...
class RefreshController {
  ...
  /// initialRefresh:When AGRefresher is init,it will call requestRefresh at once
  ///
  ...
  Future<void> requestRefresh(
    ...
    if (needMove)
      AGRefresher.ofState(position.context.storageContext)
          ?.setCanDrag(false);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (needMove) {
...
/// Controls how AGRefresher widgets behave in a subtree.the usage just like [ScrollConfiguration]
///
/// The refresh configuration determines smartRefresher some behaviours,global setting default indicator
///
/// see also:
///
/// * [AGRefresher], a widget help attach the refresh and load more function
class RefreshConfiguration extends InheritedWidget {
  ...
  ///
  /// it mostly use in some stiuation is different the other AGRefresher in App
